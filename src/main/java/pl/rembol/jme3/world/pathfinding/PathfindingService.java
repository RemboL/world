...
public class PathfindingService {
    ...
    public PathfindingService(GameState gameState) {
    ...
    public void initFromTerrain() {
        ...
        for (int x = -gameState.terrain.getTerrainSize(); x <= gameState.terrain.getTerrainSize(); ++x) {
            for (int y = -gameState.terrain.getTerrainSize(); y <= gameState.terrain
                    .getTerrainSize(); ++y) {
                float height = gameState.terrain.getGroundPosition(new Vector2f(x, y)).y;
    ...
    private void initBorders() {
        for (Integer x : clusters.keySet()) {
            for (Integer y : clusters.get(x).keySet()) {
                clusters.get(x).get(y).initBorders(gameState);
            }
        }
    }
    ...
    private void initBordersIfNeeded() {
        for (Integer x : clusters.keySet()) {
            for (Integer y : clusters.get(x).keySet()) {
                clusters.get(x).get(y).initBordersIfNeeded(gameState);
            }
        }
    }
    ...
    private boolean checkTerrainSmooth(int x, int y, float height) {
        if (x < -gameState.terrain.getTerrainSize() || x > gameState.terrain.getTerrainSize()) {
            return true;
        }
        if (y < -gameState.terrain.getTerrainSize() || y > gameState.terrain.getTerrainSize()) {
            return true;
        }
        if (Math.abs(gameState.terrain.getGroundPosition(new Vector2f(x, y)).y - height) > TERRAIN_DIFF_THRESHOLD) {
            return false;
        }
        return true;
    }
    ...
    public IExternalPath buildPath(Vector2f start, Rectangle2f target) {
        ...
        Callable<IExternalPath> worker = () -> {
            if (BresenhamAlgorithm.isDirectPathPossible(new Vector2i(start),
                    ...
                    this::isBlockFree)) {
                return new VectorPath(gameState, target.getClosest(start));
                ...
                ComplexPath complexPath = AStarAlgorithm.buildSectorPath(start,
                        startingPaths, target, targetPaths, gameState,
                        MAX_PATHFINDING_ITERATIONS);
                if (complexPath != null) {
                    return new SectorPath(complexPath, start, gameState);
                }

                return new VectorPath(gameState);
            }

        };

        return new FuturePath(gameState.threadManager.submit(Executor.FULL_PATH, worker));
    ...
    private Optional<VectorPath> buildSimplePath(Vector2i start, Rectangle2f target,
        ...
        if (BresenhamAlgorithm.isDirectPathPossible(start,
            ...
            return Optional.of(new VectorPath(path, gameState));

        } else {
            return AStarAlgorithm.buildUnitPath(start.asVector2f(), target,
                    gameState, MAX_PATHFINDING_ITERATIONS,
                    cluster::isBlockFree);
        }
    }
